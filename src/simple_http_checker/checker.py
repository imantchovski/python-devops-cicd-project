...
def check_urls(
    ...
    Args:
        urls (list[str]): A list of URLs strings to check
        timeout (int, optional): Maximum timeout in seconds to wait for each request. Defaults to 5.
    ...
    for url in urls:
        try:
            ...
            logger.error(
                f"An unexpected error occured for {url}: {e}",
                exc_info=True,
            )
